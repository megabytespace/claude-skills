...
| `validate-logo-transparent-variant.mjs` | always.md "Every nav/header/footer logo render" — when header bg luminance ≥0.85 (white/cream) AND logo has solid bg `<rect>` matching its own bg color, MUST ship transparent-bg variant `brand-mark-transparent.svg` AND CSS `picture`/`<source>` swaps based on container theme | `logo.solid_bg_on_white_nav` \| `logo.transparent_variant_missing` |
| `validate-donation-stripe-first.mjs` | always.md "Every donation/give CTA" — non-profit/donation sites MUST use Stripe Checkout (Monthly default tab + One-time tab + GiveDirectly UX preset amounts $10/$25/$50/$100/$250/$500/Custom) — NEVER PayPal/Donorbox/Network4Good as primary; Stripe Connect OAuth onboarding for new orgs | `donate.no_stripe` \| `donate.monthly_not_default` \| `donate.preset_amounts_missing` \| `donate.uses_paypal_primary` |
| `validate-no-cdn-hotlinks.mjs` | always.md "Every migrated source-site asset" — every `<img src>`, every CSS `background-image: url(...)`, every `<source srcset>` MUST resolve to R2-hosted asset under `/images/` or `/media/` — NO hotlinks to source-site CDN (`squarespace-cdn.com`, `wp-content/uploads`, `wixstatic.com`, `cdn.shopify.com`, `images.unsplash.com` for migrated content) | `asset.cdn_hotlink` \| `asset.source_cdn_referenced` |
| `validate-pointer-cursor-honesty.mjs` | always.md "Every clickable element" — `cursor: pointer` ONLY on actually-clickable elements (`<a>`, `<button>`, `[role=link]`, `[role=button]`, `[onclick]`); NEVER on decorative cards/headings/labels that aren't wired; conversely every clickable element MUST have `cursor: pointer` | `cursor.pointer_on_non_clickable` \| `cursor.missing_on_clickable` |
| `validate-modal-scroll-preservation.mjs` | always.md "Every lightbox close" — opening a modal/lightbox MUST preserve scroll position via `body { position: fixed; top: -<scrollY>px }` pattern; closing restores `window.scrollTo(0, <scrollY>)` — NEVER snap to top on close | `modal.scroll_lost_on_close` \| `modal.scroll_lost_on_open` |
| `validate-card-hover-no-flicker.mjs` | always.md "Every card hover" — card hover MUST NOT cause white-flash (default `background:white` flashing through `transform/box-shadow` transitions); set explicit `background: <bg-color>` on card, transition only `transform/box-shadow/border-color`, `will-change: transform` for GPU compositing; NEVER transition `background-color` from white→accent on hover | `card.white_flash_on_hover` \| `card.background_transition_misuse` |
| `validate-expandable-card-no-crop.mjs` | always.md "Every expandable card" — expandable/accordion cards MUST grow to fit content (`max-height: none` post-expand, `height: auto`, `overflow: visible`); NEVER `overflow: hidden` clipping content tail; siblings reflow naturally; FLIP animation pattern for smooth height transitions | `card.content_cropped` \| `card.overflow_hidden_post_expand` |
| `validate-search-input-width.mjs` | always.md "Every search input" — search inputs in blog/article/portfolio listings MUST have `min-width: 50ch` at desktop (≥1024px) AND `width: 100%` at mobile, NEVER tiny 12-character boxes; placeholder text fully visible without truncation | `search.input_too_narrow` \| `search.placeholder_truncated` |
| `validate-full-bleed-sections.mjs` | always.md "Every full-width visual section" — hero sections + featured content sections + image galleries MUST break out of parent `max-width` container via `width: 100vw; margin-left: calc(50% - 50vw)` pattern OR sit at root `<main>` level outside container; NEVER constrained to 1200px when designed to be edge-to-edge | `section.constrained_when_full_bleed_intended` \| `section.no_breakout_pattern` |
| `validate-x-not-twitter.mjs` | always.md "Every X reference" — social platform formerly known as Twitter MUST be referenced as "X" (NEVER "Twitter") AND use latest official X icon (path `M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z` in viewBox `0 0 24 24`); NEVER bird-shape Twitter icon | `social.twitter_label` \| `social.twitter_bird_icon` |
| `validate-blog-featured-images.mjs` | always.md "Every blog/article post" — every blog post MUST have a featured image rendered at top of post AND on the blog index card; if source post lacks image, DALL-E 3 per-slot prompt generates one (subject = post topic, brand palette, photographic spec, negative prompt); NEVER ship image-less blog post | `blog.featured_image_missing` \| `blog.placeholder_used` |
| `validate-comparison-table-fullbleed.mjs` | always.md "Every comparison table / data grid" — comparison tables, paperboard substrate guides, pricing matrices, spec grids MUST be full-bleed (or near-full at ≥90vw) at desktop with sticky first column for horizontal scroll on mobile (`overflow-x: auto` wrapper + `position: sticky; left: 0` on first `<th>`) | `table.constrained_layout` \| `table.no_sticky_first_column_mobile` |
| `validate-contact-preservation.mjs` | always.md "Every site rebuild SOURCE-SITE CONTACT INFO PRESERVATION" — every email/phone/address/social handle/hours/department-contact in source MUST appear on rebuild's `/contact` AND in footer; build gate diffs source-extracted contact-info JSON vs rebuild dist HTML, FAILS on any missing entity | `contact.email_missing` \| `contact.phone_missing` \| `contact.address_missing` \| `contact.social_handle_missing` \| `contact.hours_missing` \| `contact.dept_contact_missing` |
...
**2026-05-02 cycle** (driven by lonemountainglobal/njsk/nyfoldingbox three-site review — 13 critiques generalized):
